...
@app.get("/api/workflows/leaves", response_model=list[LeaveOut])
async def list_leaves(employee_id: int | None = None, session: AsyncSession = Depends(get_session)):
...
@app.post("/api/workflows/leaves", response_model=LeaveOut, status_code=201)
async def create_leave(payload: LeaveCreate, session: AsyncSession = Depends(get_session)):
...
@app.patch("/api/workflows/leaves/{leave_id}/status", response_model=LeaveOut)
...
@app.get("/api/workflows/notifications/{employee_id}", response_model=list[NotificationOut])
async def list_notifications(employee_id: int, session: AsyncSession = Depends(get_session)):
...
async def notifications_websocket(websocket: WebSocket, employee_id: int):
